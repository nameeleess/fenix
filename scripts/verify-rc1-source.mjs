...
for(const t of ['.today-header h1','.training-main-header h1','.nutrition-vnext-topbar h1','.progress-header h1']) assert.ok(appCss.includes(t),`visual coherence missing ${t}`)
...
assert.ok(progressUi.includes('type="file"')&&progressUi.includes('accept="application/json,.json"'),'restore file picker missing')
assert.ok(vite.includes('VitePWA')&&vite.includes("registerType: 'autoUpdate'")&&vite.includes("display: 'standalone'")&&vite.includes('globPatterns'),'PWA config incomplete')
...
for(const [p,expected] of Object.entries(immutable)){const actual=crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');assert.equal(actual,expected,`${p} changed outside RC1 authorization`)}
console.log('F2-RC1 source invariants: PASS (schema/deps/PWA baseline hashes preserved)')
